  ...
  <div style="text-align: center; margin: 0 auto; max-width: 1400px">
    <el-card class="box-card" shadow="hover">
      <div class="card-header" slot="header">
          ...
          <el-button type="success" style="margin-left: 220px;" @click="dialogVisible = true">添加题目</el-button>
      </div>
      <!-- 新增题目 -->
      <el-dialog title="新增题目" :visible.sync="dialogVisible" :before-close="dialoghandleClose" width="70%" show-overflow-tooltip>
        
        <el-descriptions direction="vertical" :column="1" border>
          <el-descriptions-item label="请输入题目名称">
           <el-input v-model="topicName"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="请输入题目内容">
            <v-md-editor v-model="content" height="400px" mode="edit"></v-md-editor>
          </el-descriptions-item>
          <el-descriptions-item label="输入案例(每组以+号分割) eg:(1+2+3)">
            <el-input v-model="input"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="输出案例(每组以+号分割) eg:(hello+world+code)">
            <el-input v-model="output"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="添加标签">
            <el-tag  :key="tag"  v-for="tag in dynamicTags"  closable  :disable-transitions="false"  @close="handleClose(tag)">  {{tag}}</el-tag><el-input  class="input-new-tag"  v-if="inputVisible"  v-model="inputValue"  ref="saveTagInput"  size="small"  @keyup.enter.native="handleInputConfirm"  @blur="handleInputConfirm">
</el-input>
<el-button v-else class="button-new-tag" size="small" @click="showInput">+ New Tag</el-button>
      ...
      <!-- 编辑题目 -->
        ...
        <el-descriptions direction="vertical" :column="1" border>
          ...
          <el-descriptions-item label="添加标签">
            <el-tag  :key="tag"  v-for="tag in form.label"  closable  :disable-transitions="false"  @close="handleClose2(tag)">  {{tag}}</el-tag><el-input  class="input-new-tag"  v-if="inputVisible2"  v-model="inputValue2"  ref="saveTagInput"  size="small"  @keyup.enter.native="handleInputConfirm2"  @blur="handleInputConfirm2">
</el-input>
...
import axios from '@/http';
  ...
  export default {
    ...
    methods: {
      ...
      delProblem(ID){
      axios.post('/api/admin/topic/del', {
        id: ID,
      },{  
        headers: {  
        'token': localStorage.getItem('token'),
      ...
      addProblem(){
      axios.post('/api/admin/topic/add', {
      ...
      changeProblem(){
      axios.post('/api/admin/topic/change', {
        id: this.form.id,
      ...
      adminShowBack(id) {
      axios.get(`/api/admin/topic/show/${id}`,{  
        headers: {  
        'token': localStorage.getItem('token'),
        } 
      }).then(res => {
